"""
API principal de FastAPI para análisis de tránsito peatonal.
"""
...
app = FastAPI(
    title="People Crossing Analysis API",
    description="API para análisis peatonal usando YOLOv26s",
    version="1.0.0"
)
...
async def root():
    ...
    return {
        "message": "People Crossing Analysis API",
        "version": "1.0.0",
        "model": "YOLOv26s-seg",
        "endpoints": {
            "upload": "/api/upload",
...
async def health_check():
    ...
    model_is_file = model_path.is_file() if model_exists else False
    model_size_bytes = model_path.stat().st_size if model_is_file else 0
    model_valid = model_exists and model_is_file and model_size_bytes >= 1024

    return {
        "status": "healthy" if model_valid else "unhealthy",
        "model_loaded": model_valid,
